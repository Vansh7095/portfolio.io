...
if (textAnimation) {
    const texts = ['Web & Mobile App Designer', 'Creative Coder', 'Visionary', 'Curious', 'Builder-Minded'];
    let textIndex = 0;
    let charIndex = 0;
...
function revealOnScroll() {
    revealElements.forEach(element => {
        ...
        if (elementTop < windowHeight - 100) {
            ...
            // Animate skill bars when skill card is revealed
            if (element.classList.contains('skill-card')) {
                const progress = element.querySelector('.skill-progress');
                const percent = element.querySelector('.skill-percent');
...
// ====== PROJECT FILTER (from projects.html) ======
